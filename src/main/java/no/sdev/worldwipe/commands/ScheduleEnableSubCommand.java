...
public class ScheduleEnableSubCommand extends CommandBase {

    public ScheduleEnableSubCommand() {
        super("enable", "Enable scheduled wipes");
    ...
    protected void executeSync(@Nonnull CommandContext context) {
        if (context.isPlayer()) {
            if (!WorldWipePermissions.hasCommand(context.sender(), "schedule.enable")) {
                context.sendMessage(Message.raw("You don't have permission. ("
                        + WorldWipePermissions.command("schedule.enable") + ")"));
                return;
            }
        ...
        boolean updated = plugin.updateSchedulingEnabled(true);
        if (!updated) {
            context.sendMessage(Message.raw("Failed to enable scheduling."));
            return;
        }
        context.sendMessage(Message.raw("Scheduling enabled."));
    }
}
